...
function secret() {
  ...
  if (!value || value.length < 32)
    throw new Error("VISUALIZER_SESSION_SECRET must be at least 32 characters");
  return value;
}
...
export function createSessionCookie() {
  ...
  return {
    id,
    value,
    header: `${SESSION_COOKIE}=${value}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=7200`,
  };
}
...
export function readSession(request: Request) {
  ...
  const id = raw.slice(0, split),
    provided = raw.slice(split + 1);
  return /^[0-9a-f-]{36}$/i.test(id) && safeEqual(provided, signature(id))
    ? id
    : null;
}

export function ownerPrefix(sessionId: string) {
...
export function uploadPath(
  ...
  return `visualizer/${ownerPrefix(sessionId)}/${requestId}/${kind}.${extension}`;
}
...
export function logTransfer(details: {
  requestId: string;
  stage: string;
  ...
  console.info(
    JSON.stringify({
      event: "nest_visualizer_transfer",
      requestId: details.requestId,
      stage: details.stage,
      productId: details.productId || null,
      requestBytes: details.requestBytes ?? null,
