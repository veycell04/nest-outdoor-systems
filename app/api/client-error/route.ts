...
export async function POST(request:Request){
  ...
  const input=body as Record<string,unknown>,message=sanitizeError(typeof input.message==="string"?input.message:"Client render error"),path=typeof input.path==="string"&&/^\/[A-Za-z0-9/_-]{0,200}$/.test(input.path)?input.path:"/";
  recent.push(now);reports.set(key,recent);logFailure({requestId,stage:"client_render",productId:null,startedAt,status:500,error:new Error(`${message} at ${path}`)});
