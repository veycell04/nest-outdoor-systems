...
export async function GET(request: Request) {
  ...
  if (
    ...
    return customerError(
      "This concept link expired. Generate it again for a new private link.",
      requestId,
      403,
    );
  try {
    ...
    if (!result || result.statusCode !== 200)
      return customerError(
        "The concept is no longer available.",
        requestId,
        404,
      );
    ...
    return customerError(
      "The private concept could not be loaded.",
      requestId,
      502,
    );
  }
