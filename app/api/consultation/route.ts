...
export async function POST(request:Request){
  ...
  try{
    const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({from:process.env.CONSULTATION_FROM_EMAIL,to:[process.env.CONSULTATION_TO_EMAIL||"hello@nestpergola.com"],reply_to:email,subject:`NEST project inquiry — ${product.label}`,text:[`Name: ${name}`,`Email: ${email}`,`Phone: ${phone||"Not provided"}`,`ZIP: ${zip||"Not provided"}`,`Product: ${product.label}`,`Message: ${message||"Not provided"}`,"",context?`Visualizer details:\n${context}`:"No visualizer details provided."].join("\n")})});
